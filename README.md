...
burrow agent claude install              # wire Claude Code to burrow-agent (any other agent: `burrow agent <tool>` prints the rules to set)
```
...
See [docs/getting-started.md](docs/getting-started.md) for the full walkthrough. Claude Code is the
one agent with built-in wiring today; for any other, `burrow agent <tool>` prints the exact
allow/deny rules to set by hand — `burrow-agent` is a plain binary, so any agent that can run a
...
single node needs headroom above the platform and control-plane baseline. It is optional and off
the default path — building and pushing images with your own tooling, then deploying by reference,
remains the default — and it carries the limits noted in
...
Burrow follows semver from v0.1 toward v1.0. This table never lags the code. For what is built
today versus what is decided but not yet built, see
[docs/CAPABILITIES.md](docs/CAPABILITIES.md).
...
- [docs/CAPABILITIES.md](docs/CAPABILITIES.md): "can Burrow do X?" — every capability, the
  command that reaches it, its sharp limits, and what is decided but not yet built.
