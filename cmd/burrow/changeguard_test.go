...
var commandClasses = map[string]commandClass{
	...
	"app secret mounts":     {class: classReads, why: "lists which secret KEYS are read as files, and where"},
	...
	"app secret mount":      {class: classChanges, why: "projects a secret key into a file and re-applies the workload", args: []string{"app", "secret", "mount", "web", "K"}},
	"app secret unmount":    {class: classChanges, why: "stops projecting that key as a file and re-applies the workload", args: []string{"app", "secret", "unmount", "web", "K"}},
	...
	"addon detach":          {class: classChanges, why: "takes an app's database away from it", args: []string{"addon", "detach", "postgres", "web", "--confirm"}},
