...
// probeContextFn probes a single kube context for an installed burrowd in the given control-plane
...
// a fake probe and exercise `burrow env list --discover` without a real cluster, the way `burrow
// version` classifies a cluster from the burrowd Deployment image (ADR-0036).
var probeContextFn = func(ctx context.Context, kubeconfig, kubeContext, namespace string) (string, error) {
...
// probeRow is one context's probe outcome, rendered as a table row. installed drives both the
// VERSION cell and whether discovery registers a handle for the context.
...
// discoverEnvironments walks every kubeconfig context, probes each for an installed Burrow, and
// registers a local handle for each installed context that has none yet (ADR-0036). It reads
// clusters but mutates only ~/.burrow/config; it never changes a cluster. It returns the probe rows
// (for the report) and the names it registered. This is the networked, config-mutating core behind
// `burrow env list --discover`; bare `burrow env list` never calls it.
func discoverEnvironments(ctx context.Context, kubeconfig, namespace string) (rows []probeRow, added []string, err error) {
	...
	for _, c := range contexts {
		...
		img, perr := probeContextFn(probeCtx, kubeconfig, c.Name, namespace)
		cancel()
		row := probeRow{context: c.Name, cluster: c.Cluster}
	...
	// already has a handle is left untouched, so re-running discovery adds only what is new.
	...
	return rows, added, nil
}

// writeDiscoverReport prints the probe outcomes as an aligned CONTEXT/CLUSTER/BURROWD/VERSION table
// followed by an outcome-aware summary of what discovery registered, ahead of the handle list that
// `burrow env list --discover` prints below it. It is the human (non-JSON) rendering of a discovery
// pass.
func writeDiscoverReport(w io.Writer, rows []probeRow, added []string, namespace string) {
	if len(rows) == 0 {
...
// so discovery does not register a duplicate.
...
// discovery never collides with an existing handle name (e.g. one a different context already claimed).
