...
func (e *Engine) AddDomain(ctx context.Context, req AddDomainRequest) (DomainResult, error) {
	...
	if err := e.recordDecision(ctx, auditOpDNSWrite, host, args, GuardrailDNSWrite,
		pol.evaluateGuardrail(ctx, GuardrailScope{}, "domain add", GuardrailDNSWrite, req.Confirm,
...
func (e *Engine) RemoveDomain(ctx context.Context, req RemoveDomainRequest) (DomainResult, error) {
	...
	if err := e.recordDecision(ctx, auditOpDNSDelete, host, args, GuardrailDNSDelete,
		pol.evaluateGuardrail(ctx, GuardrailScope{}, "domain remove", GuardrailDNSDelete, req.Confirm,
