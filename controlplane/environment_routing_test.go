...
func TestPerEnvironmentGuardrailGatesOnlyThatEnv(t *testing.T) {
	...
	if err := e.SetGuardrail(asOperator(ctx), cp.GuardrailScope{}, "", cp.GuardrailAppDelete, cp.DispositionAllow); err != nil {
		t.Fatalf("SetGuardrail(global): %v", err)
	}
	if err := e.SetGuardrail(asOperator(ctx), cp.GuardrailScope{Env: "staging"}, "", cp.GuardrailAppDelete, cp.DispositionDeny); err != nil {
...
func TestSetGuardrailEnvValidation(t *testing.T) {
	...
	if err := e.SetGuardrail(asOperator(ctx), cp.GuardrailScope{Env: "ghost"}, "", cp.GuardrailAppDelete, cp.DispositionDeny); !errors.Is(err, cp.ErrNotFound) {
		t.Errorf("SetGuardrail(ghost) = %v, want ErrNotFound", err)
	}
	// A cluster-level guardrail cannot be scoped to an environment.
	if err := e.SetGuardrail(asOperator(ctx), cp.GuardrailScope{Env: "staging"}, "", cp.GuardrailAddonInstall, cp.DispositionDeny); !errors.Is(err, cp.ErrInvalid) {
		t.Errorf("SetGuardrail(staging, addon.install) = %v, want ErrInvalid (cluster-level)", err)
	}
	if err := e.SetGuardrail(asOperator(ctx), cp.GuardrailScope{Env: "staging"}, "", cp.GuardrailDNSWrite, cp.DispositionDeny); !errors.Is(err, cp.ErrInvalid) {
...
func TestClusterLevelGuardrailIgnoresEnv(t *testing.T) {
	...
	if err := e.SetGuardrail(asOperator(ctx), cp.GuardrailScope{}, "", cp.GuardrailAddonInstall, cp.DispositionDeny); err != nil {
