...
func TestDeployDeniedForOneAppOnly(t *testing.T) {
	...
	if err := e.SetGuardrail(asOperator(ctx), cp.GuardrailScope{Env: cp.DefaultEnvironment, Name: "control-plane"}, "", cp.GuardrailAppDeploy, cp.DispositionDeny); err != nil {
...
func TestNameScopedDispositionSurvivesTheEnvironmentTier(t *testing.T) {
	...
	if err := e.SetGuardrail(asOperator(ctx), cp.GuardrailScope{Env: "staging"}, "", cp.GuardrailAppDelete, cp.DispositionAllow); err != nil {
		t.Fatalf("SetGuardrail(staging, app.delete, allow): %v", err)
	}
	if err := e.SetGuardrail(asOperator(ctx), cp.GuardrailScope{Env: "staging", Name: "control-plane"}, "", cp.GuardrailAppDelete, cp.DispositionDeny); err != nil {
...
func TestAddonGuardrailScopesToOneInstance(t *testing.T) {
	...
	if err := e.SetGuardrail(asOperator(ctx), cp.GuardrailScope{Env: "staging", Name: instance}, "", cp.GuardrailAddonInstall, cp.DispositionDeny); err != nil {
...
func TestSetGuardrailNameValidation(t *testing.T) {
	...
	err := e.SetGuardrail(asOperator(ctx), cp.GuardrailScope{Name: "website"}, "", cp.GuardrailAppRun, cp.DispositionDeny)
	...
	err = e.SetGuardrail(asOperator(ctx), cp.GuardrailScope{Env: "staging", Name: "website"}, "", cp.GuardrailDNSWrite, cp.DispositionAllow)
	if !errors.Is(err, cp.ErrInvalid) {
	...
	if err := e.SetGuardrail(asOperator(ctx), cp.GuardrailScope{Env: "staging", Name: "web.site"}, "", cp.GuardrailAppRun, cp.DispositionDeny); !errors.Is(err, cp.ErrInvalid) {
	...
	if err := e.SetGuardrail(asOperator(ctx), cp.GuardrailScope{Env: "ghost", Name: "website"}, "", cp.GuardrailAppRun, cp.DispositionDeny); !errors.Is(err, cp.ErrNotFound) {
...
func TestGuardrailsForNameReportsTheTierThatAnswered(t *testing.T) {
	...
	if err := e.SetGuardrail(asOperator(ctx), cp.GuardrailScope{Env: "staging", Name: "website"}, "", cp.GuardrailAppRun, cp.DispositionDeny); err != nil {
