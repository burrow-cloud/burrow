...
func TestBackupRestoreAuditRedacted(t *testing.T) {
	...
	for _, row := range rows {
		...
		for key, v := range row.Args {
			// Per operation: an install row names the type and image, a backup or restore row names
			// the instance it acted on (ADR-0091 §4). None of them carries a credential.
			allowed := map[string]bool{"addon": true, "app": true, "env": true, "instance": true, "backup": true, "type": true, "image": true, "kind": true, "destination": true}
			if !allowed[key] {
