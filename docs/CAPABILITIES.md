...
| `addon.detach` | detaching an app from an add-on, taking away its access (the data is kept) | `confirm` | no | add-on instance |
...
guardrail names two things — `addon detach <addon> <app>` releases one app's database on one instance —
...
`addon restore`, `addon restore-instance`, `config provider add`, `config registry login`, `agent <tool> install`,
...
| `burrow addon detach postgres <app> --env staging` | `POST /v1/addons/detach/env/{env}` | Taken the default environment's database away from the app still using it |
