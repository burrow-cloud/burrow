...
| Detach | `burrow addon detach postgres <app> [--env]` | Removes the variable the attachment was written under (`DATABASE_URL` unless it named another), rolls the app off the database, then closes it to new connections and deletes the `Database` and `DatabaseRole` objects with a `delete` reclaim policy — CloudNativePG runs the `DROP DATABASE` and `DROP ROLE` **on that environment's instance**. **Destructive**; confirm-gated. |
