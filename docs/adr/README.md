...
| [0046](0046-bundled-in-cluster-registry.md) | A bundled in-cluster registry for quick starts — ship zot (lightweight, OCI-native) as an optional in-cluster registry so a solo dev deploys without an external account; the laptop pushes over a NodePort (auth + TLS required, free on every environment, no billable LB), the kubelet pulls in-cluster via a containerd registry-mirror, and burrowd never touches an image (ADR-0040 preserved). A quick-start convenience, not the spine: the external-registry path stays first-class and is the hardened recommendation. Harbor rejected as too heavy for the 2GB floor; control-plane-mediated push rejected | Accepted |
