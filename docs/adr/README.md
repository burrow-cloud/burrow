...
| [0052](0052-pull-based-passive-deploy.md) | Pull-based passive deploy — a semver-scoped registry watcher on the guarded path. burrowd polls the registry for new tags of an app's image and, when a tag is within the app's per-environment auto-update scope (`off` / `patch` / `minor` (default) / `major`), fires the same explicit guarded deploy (same rollout, record, rollback, audit). Default is `minor` (patch+minor land automatically); `major` is available for hands-off updates but not the default, and a major above a lower scope is surfaced as an *available upgrade* to take with an explicit `burrow app deploy`. Polling is conservative (~5 min) because the explicit deploy is always the immediate path. Outbound-only, so it works on the private/NAT'd/firewalled clusters a push-from-CI model cannot reach (ADR-0014). Rejects mutable-tag auto-pull (no control) and push-from-CI as primary; realizes ADR-0007's optional passive mode without replacing the explicit spine; setting the scope stays a human action (ADR-0038), the agent observes and Burrow guides the agent to semver tags without enforcing | Accepted |
