...
| [0044](0044-single-vps-k3s-cluster.md) | Provision a single-VPS k3s cluster — stand up k3s + Burrow on one cheap VPS (~$5/mo, node IP = free servicelb LoadBalancer, no cloud LB) for the self-hoster tier; open question under investigation is the install mechanism (SSH from the laptop vs an installer run on the VPS) | Proposed |
