...
func NewServer(clientFor ClientForContext, kubeconfig, version string) *sdk.Server {
	...
	sdk.AddTool(s, &sdk.Tool{
		Name:        "burrow_deploy",
		Description: "Deploy an application to the cluster by container image reference. To deploy an app from source: infer the app name from the working directory (the folder name) unless the user names one; build a container image from the working directory with your OWN container tooling (for example `docker build`) and push it to a registry the cluster can pull from; then call this tool with that image reference. Do NOT run the `burrow` CLI to build or deploy — you operate through these tools, not the CLI. If the image is already built and pushed, just pass its reference. Only the reference and small metadata are sent, never code (the image moves through the registry). Config is NOT passed here: an app's config is a separate, app-global store sourced at deploy time, so set any config vars the release needs with burrow_config_set BEFORE deploying — the new release then boots with it on first start. (burrow_config_set with no_restart=true followed by burrow_deploy is a single restart.) For SECRETS (DB URLs, API keys), do not put values in config and do not paste secret values into this conversation: ask the user to run `burrow app secret set <app> KEY=VALUE` themselves BEFORE deploying, then confirm the key with burrow_secret_list. If the image lives in a PRIVATE registry, the cluster needs pull credentials to fetch it — a one-time credential step that never goes over MCP (there is deliberately no tool for it), so ask the user to run `burrow config registry login <host>` themselves at their terminal BEFORE deploying (e.g. `burrow config registry login ghcr.io`), then deploy; do not try to make the image public or wire a credential yourself. Returns the new release and the release it superseded (the rollback handle), plus the environment it acted in. Pass env to target a local environment handle, such as staging or prod (default follows the current kube context with the default environment); this is how you deploy the same app to staging versus prod. Use burrow_environments to see the handles you can name; context is a low-level override for a cluster with no handle. Deploying is allowed by default, but an operator may configure the app.deploy guardrail to hold it for confirmation (e.g. confirm in prod) or deny it; when held, the error says so — ask the user, then retry with confirm set to true ONLY after the user approves, never on your own.",
