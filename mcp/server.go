...
// burrowInstructions is the MCP server-level instructions string, delivered to the client at
// connect time (the InitializeResult.instructions field) with no tool round-trip. It orients the
// agent to Burrow as a whole — what it is, that operating happens through these tools and never the
// CLI, the typical deploy flow, how guardrails and confirmation work, and the human-only credential
// and setup steps — so the cross-cutting guidance lives in one always-loaded place instead of being
// repeated across every tool description. It must stay accurate to the shipped tool set and honest
// about behavior (ADR-0009): no invented tools, no secret values.
const burrowInstructions = `Burrow lets you operate the user's real applications on their own Kubernetes cluster. You are the operator; these tools are the remote control — the control plane behind them holds the cluster credentials and enforces the guardrails, so drive everything through these tools, never the ` + "`burrow`" + ` CLI (the CLI is the human's own tool).

Code never travels over MCP. To ship an app from source, build a container image with your own tooling (e.g. docker build) and push it to a registry the cluster can pull from, then deploy by image reference; only the reference and small metadata cross MCP, never code.
...
Environments: every app lives in a specific environment (e.g. prod, staging), and the target is never implicit. Before any mutating operation, call burrow_environments to see the handles and pass the env argument explicitly (e.g. env: prod) rather than relying on a default; when more than one environment is registered, confirm the intended one with the user before you deploy, roll back, scale, or delete — an auto-generated handle name (e.g. jolly-marmot) is not self-evidently the production target. Then stay on that one environment for the whole task: if a call fails — a transient error, a timeout, or an unreachable control plane — surface the error and stop, and NEVER retry the same operation against a different environment or switch environments to work around a failure. Changing the target environment is a deliberate act that needs the user's explicit go-ahead. Survey before you act with the read-only tools: burrow_cluster (capabilities), burrow_status, burrow_apps, and burrow_reachability.`
...
func NewServer(clientFor ClientForContext, kubeconfig, version string) *sdk.Server {
	...
	sdk.AddTool(s, &sdk.Tool{
		Name:        "burrow_expose",
		Description: "Make a deployed application reachable from outside the cluster at a hostname, by creating a Service and an Ingress. Public exposure is held for confirmation by a guardrail by default; when held, the error says so — ask the user, then retry with confirm set to true. Reachability also needs an ingress controller and DNS pointing the host at the cluster. If the cluster is not set up for public reachability (no ingress controller, or no cert-manager when tls is set), the error names each missing prerequisite and the exact burrow command that provisions it; run that command rather than inspecting the cluster directly. Before recommending ingress install, to make the app publicly reachable present a LoadBalancer: it gives a public IP to point DNS at — have the human run `burrow cluster ingress install --expose loadbalancer --approve` (on a cloud provider this is a billable cloud load balancer, which is why `--approve` is required). When no DNS provider is configured, do not default to a manual registrar step: ask the user which provider hosts the domain and, if Burrow supports it (the error and burrow_providers name the supported types), offer to have them configure it with `burrow config provider add <type>` so Burrow automates the record — provider setup is a human/operator step (a credential, never over MCP).",
	...
	sdk.AddTool(s, &sdk.Tool{
		...
		Description: "List the configured cloud providers and the capabilities each serves (e.g. dns), so you know which provider name to pass for an operation like burrow_domain_add. Read-only: provider credentials are configured by the operator via the CLI, never by an agent. When an operation reports no DNS provider is configured, its guidance names the supported provider types (e.g. cloudflare, digitalocean) to choose from before the operator runs `burrow config provider add <type>`.",
	...
	sdk.AddTool(s, &sdk.Tool{
		Name:        "burrow_cluster",
		Description: "See what this cluster can do before you change anything (ADR-0034): a neutral, read-only report of its capabilities, read live. Tells you whether an ingress controller is installed and which IngressClass to use, whether there is a default StorageClass (and its name) for persistent volumes, whether Service type=LoadBalancer is likely supported (from a cloud provider, k3s servicelb, or MetalLB — only a cloud load balancer is billable) or no LoadBalancer provider is present, whether cert-manager is installed (for TLS), the cloud provider, and whether a DNS provider is configured. Use it to survey a cluster and explain its state — and to know whether an operation like exposing an app or requesting a certificate will work — before doing anything. When an ingress controller or cert-manager is missing, the remediation is not an agent action: recommend the human run `burrow cluster ingress install` (it installs whichever pieces are missing). To make an app publicly reachable, present a LoadBalancer: it gives a public IP to point DNS at — have the human run `burrow cluster ingress install --expose loadbalancer --approve` (on a cloud provider this is a billable cloud load balancer, which is why `--approve` is required). Read-only: it changes nothing and returns no secret. Pass context to survey a specific cluster/environment (default the current one); this is how you compare prod versus staging.",
