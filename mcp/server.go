...
Human, CLI-only steps never go over MCP: relay the exact command and ask the USER to run it at their own terminal, and never ask them to paste a secret value into the conversation. These are: registry pull credentials for a private image (` + "`burrow config registry login <host> -u <user>`" + `, which prompts for the token with input hidden); app secrets like DB URLs and API keys (` + "`burrow app secret set <app> KEY=VALUE`" + `); DNS/cloud provider credentials (` + "`burrow config provider add <type>`" + `); and cluster setup (` + "`burrow install`" + `, ` + "`burrow upgrade`" + `, ` + "`burrow cluster ingress install`" + `). When a tool result says a prerequisite is missing, relay the exact command it names.

Environments: per-app operations target a local environment handle (e.g. prod, staging) via the env argument; use burrow_environments to see the handles you can name. Survey before you act with the read-only tools: burrow_cluster (capabilities), burrow_status, burrow_apps, and burrow_reachability.`
